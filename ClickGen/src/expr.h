...
class ConstantExpr : public Expr {
  public:
    ...
    static std::shared_ptr<ConstantExpr>
    create(std::shared_ptr<PopulateCtx> ctx, IntTypeID type_id_below);
...
class ScalarVarUseExpr : public VarUseExpr {
  public:
    ...
    std::string getScalarVarName(std::shared_ptr<EmitCtx> ctx){return value->getName(ctx);}

    bool propagateType() final { return true; }
    EvalResType evaluate(EvalCtx &ctx) final;
    EvalResType rebuild(EvalCtx &ctx) final;

    void emitLocal(std::shared_ptr<EmitCtx> ctx, std::ostream &stream,
              std::string offset = "") {
      // std::cout<<"The click_name = "<<value->getName(ctx)<<std::endl;
      // if (std::regex_match(value->getName(ctx), std::regex("(ip|tcp|udp)->.*"))) {
      //   std::cout<<"True"<<std::endl;
      // }
      if (std::regex_match(value->getName(ctx), std::regex("(ip|tcp|udp)->.*")) && !rand_val_gen->emit_click){
        stream << offset <<rand_val_gen->getNNName(value->getName(ctx));
      } else {
      ...
      // if (value->getKind()==DataKind::VAR){
      //   if (value->getName(ctx).substr(0, 2)=="l_" && std::static_pointer_cast<ScalarVar>(value)->defined==false){
      //     std::static_pointer_cast<ScalarVar>(value)->defined=true;
      //     stream << offset << "uint32_t "<<value->getName(ctx);
      //   } else {
      //     stream << offset << value->getName(ctx);
      //   }
      // } else {
      //   ERROR("Wrong datakind var");
      // }
    };
...
class ArithmeticExpr : public Expr {
  public:
    ...
    static std::shared_ptr<Expr> create(std::shared_ptr<PopulateCtx> ctx, IntTypeID type_id_below, bool flagIf=false, bool minusPlus=false, bool isLogicNot=false);
...
class UnaryExpr : public ArithmeticExpr {
  public:
    ...
    static std::shared_ptr<UnaryExpr> create(std::shared_ptr<PopulateCtx> ctx, IntTypeID type_id_below, bool flagIf=false);
...
class BinaryExpr : public ArithmeticExpr {
  public:
    ...
    static std::shared_ptr<BinaryExpr> create(std::shared_ptr<PopulateCtx> ctx, IntTypeID type_id_below, bool flagIf=false);
...
class AssignmentExpr : public Expr {
  public:
    AssignmentExpr(std::shared_ptr<Expr> _to, std::shared_ptr<Expr> _from, AssignKind _assign_kind,
                   bool _taken = true)
        : to(std::move(_to)), from(std::move(_from)), assign_kind(std::move(_assign_kind)), taken(_taken) {}
...
class MinMaxCallBase : public LibCallExpr {
  ...
  protected:
    MinMaxCallBase(std::shared_ptr<Expr> _a, std::shared_ptr<Expr> _b,
                   LibCallKind _kind);
    static std::shared_ptr<LibCallExpr>
    createHelper(std::shared_ptr<PopulateCtx> ctx, LibCallKind kind, IntTypeID type_id_below);
...
class MinCall : public MinMaxCallBase {
  public:
    ...
    static std::shared_ptr<LibCallExpr>
    create(std::shared_ptr<PopulateCtx> ctx, IntTypeID type_id_below) {
        return createHelper(std::move(ctx), LibCallKind::MIN, type_id_below);
    }
...
class MaxCall : public MinMaxCallBase {
  public:
    ...
    static std::shared_ptr<LibCallExpr>
    create(std::shared_ptr<PopulateCtx> ctx, IntTypeID type_id_below) {
        return createHelper(std::move(ctx), LibCallKind::MAX, type_id_below);
    }
    // static void emitCDefinition(std::shared_ptr<EmitCtx> ctx,
    //                             std::ostream &stream, std::string offset = "") {
