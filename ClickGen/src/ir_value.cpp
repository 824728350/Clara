...
template <typename T> static IRValue plusPlusOperator(IRValue &operand) {
    IRValue ret(operand.getIntTypeID());
    if (operand.hasUB())
        return ret;
    ret.getValueRef<T>() = operand.getValueRef<T>()+1;
    ret.setUBCode(UBKind::NoUB);
    return ret;
}

IRValue IRValue::operator++() { UnaryOperatorImpl(plusPlusOperator); }

//////////////////////////////////////////////////////////////////////////////

template <typename T> static IRValue minusMinusOperator(IRValue &operand) {
    IRValue ret(operand.getIntTypeID());
    if (operand.hasUB())
        return ret;
    ret.getValueRef<T>() = operand.getValueRef<T>()-1;
...
IRValue IRValue::operator--() { UnaryOperatorImpl(minusMinusOperator); }
...
divModImpl(IRValue &lhs, IRValue &rhs, std::function<T(T &, T &)> op) {
    if (rhs.getIntTypeID() != lhs.getIntTypeID()) {
        // std::cout << str_IntTypeID[lhs.getIntTypeID()] << " "<< str_IntTypeID[rhs.getIntTypeID()] << std::endl;
        // std::cout << getIntTypeIDStr(lhs.getIntTypeID()) << " "<<getIntTypeIDStr(rhs.getIntTypeID()) << std::endl;
