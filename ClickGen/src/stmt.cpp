...
StmtBlock::generateStructure(std::shared_ptr<GenCtx> ctx, IRNodeKind block_type) {
    std::vector<std::shared_ptr<Stmt>> stmts;

    auto gen_policy = ctx->getGenPolicy();
    size_t stmt_num;

    if (block_type == IRNodeKind::STUB) {
       stmt_num = rand_val_gen->getRandId(gen_policy->stub_scope_stmt_num_distr);
    } else if (block_type == IRNodeKind::LOOP_SEQ || block_type == IRNodeKind::LOOP_NEST) {
       stmt_num = rand_val_gen->getRandId(gen_policy->for_scope_stmt_num_distr);
    } else if (block_type == IRNodeKind::IF_ELSE) {
       stmt_num = rand_val_gen->getRandId(gen_policy->if_scope_stmt_num_distr);
    }
    ...
    for (size_t i = 0; i < stmt_num; ++i) {
        // TODO: use fake if distirbution
        IRNodeKind stmt_kind = rand_val_gen->getRandId(gen_policy->stmt_kind_struct_distr);
        // if (rand_val_gen->less_if) {
        //     stmt_kind = rand_val_gen->getRandId(gen_policy->stmt_kind_struct_distr2);
        // } else  {
        //     stmt_kind = rand_val_gen->getRandId(gen_policy->stmt_kind_struct_distr);
        // }
        ...
        // std::cout<<"cur = " << rand_val_gen->stmtNumStack.top() <<", avail="<<rand_val_gen->totalAvailHF<<std::endl;
        fallback |= rand_val_gen->stmtNumStack.top() >= rand_val_gen->totalAvailHF;
        ...
        // stmt)
        fallback |= stmt_kind == IRNodeKind::LOOP_NEST && (stats.getStmtNum() + 3 >= gen_policy->stmt_num_lim);

        fallback |= (rand_val_gen->varAllUsed && rand_val_gen->hfAllUsed);

        if (fallback)
            break;

        if (stmt_kind == IRNodeKind::LOOP_SEQ && ctx->getLoopDepth() < gen_policy->loop_depth_limit &&
                rand_val_gen->stmtNumStack.top() + gen_policy->for_scope_stmt_max_num <= rand_val_gen->totalAvailHF &&
                stats.getStmtNum() + 1 < gen_policy->stmt_num_lim) {
            std::cout << "LOOP_SEQ" << std::endl;
            new_stmt = LoopSeqStmt::generateStructure(ctx);
            std::cout << "LOOP_SEQ_END" << std::endl;
        }
        else if (stmt_kind == IRNodeKind::LOOP_NEST && ctx->getLoopDepth() + 2 <= gen_policy->loop_depth_limit &&
                rand_val_gen->stmtNumStack.top() + gen_policy->for_scope_stmt_max_num <= rand_val_gen->totalAvailHF &&
                stats.getStmtNum() + 1 < gen_policy->stmt_num_lim) {
            std::cout << "LOOP_NEST" << std::endl;
            new_stmt = LoopNestStmt::generateStructure(ctx);
            std::cout << "LOOP_NEST_END" << std::endl;
        }
        else if (stmt_kind == IRNodeKind::IF_ELSE && ctx->getIfElseDepth() + 1 <= gen_policy->if_else_depth_limit &&
                rand_val_gen->stmtNumStack.top() + gen_policy->if_scope_stmt_max_num <= rand_val_gen->totalAvailHF &&
                stats.getStmtNum() + 1 < gen_policy->stmt_num_lim) {
            std::cout << "IF_ELSE" << std::endl;
            new_stmt = IfElseStmt::generateStructure(ctx);
            std::cout << "IF_ELSE_END" << std::endl;
        }
        else
        {   // std::cout << " enter substmt" << std::endl;
            new_stmt = StubStmt::generateStructure(ctx);
            stats.addStmt();
            uint32_t lastNum = rand_val_gen->stmtNumStack.top();
            rand_val_gen->stmtNumStack.pop();
            rand_val_gen->stmtNumStack.push(lastNum+1);
        }
...
void StmtBlock::populate(std::shared_ptr<PopulateCtx> ctx) {
    ...
    for (auto &stmt : stmts) {
        if (stmt->getKind() != IRNodeKind::STUB) {
            if (stmt->getKind() == IRNodeKind::IF_ELSE) {
                std::set<std::string> newHF;
                rand_val_gen->hfCtxStack.push(newHF);
                stmt->populate(ctx);
            ...
            std::shared_ptr<Stmt> new_stmt;
            IRNodeKind new_stmt_kind = rand_val_gen->getRandId(gen_pol->stmt_kind_pop_distr);
...
ScopeStmt::generateStructure(std::shared_ptr<GenCtx> ctx, IRNodeKind block_type) {
    // TODO: will that work?
    auto new_scope = std::make_shared<ScopeStmt>();
    auto stmt_block = StmtBlock::generateStructure(std::move(ctx), block_type);
...
LoopSeqStmt::generateStructure(std::shared_ptr<GenCtx> ctx) {
    ...
    for (size_t i = 0; i < loop_num; ++i) {
        ...
        auto new_loop_body = ScopeStmt::generateStructure(new_ctx, IRNodeKind:: LOOP_SEQ);
...
LoopNestStmt::generateStructure(std::shared_ptr<GenCtx> ctx) {
    ...
    nest_depth = std::min(gen_pol->loop_depth_limit - ctx->getLoopDepth(), nest_depth);
    ...
    new_loop_nest->addBody(ScopeStmt::generateStructure(new_ctx, IRNodeKind::LOOP_NEST));
...
void IfElseStmt::emit(std::shared_ptr<EmitCtx> ctx, std::ostream &stream,
    ...
    auto gen_pol = gen_ctx->getGenPolicy();
    // if (gen_pol->max_arith_depth == 2){
    //     stream << offset << "if ";
    // } else {
    //     stream << offset << "if (";
    // }
    ...
    if (else_br.use_count() != 0) {
        stream << offset << "else \n";
        stream << offset << "{\n";
        // stream << offset << "else_bar="<<else_br->getStmts().size()<<"\n";
...
IfElseStmt::generateStructure(std::shared_ptr<GenCtx> ctx) {
    ...
    rand_val_gen->stmtNumStack.push((uint32_t)0);
    // std::cout<<"new_ctx = "<<new_ctx->getIfElseDepth()<<std::endl;
    // std::cout<<"old_ctx = "<<ctx->getIfElseDepth()<<std::endl;
    auto then_br = ScopeStmt::generateStructure(new_ctx, IRNodeKind::IF_ELSE);
    bool else_br_exist = rand_val_gen->getRandId(gen_pol->else_br_distr);
    std::shared_ptr<ScopeStmt> else_br;
    if (else_br_exist)
        else_br = ScopeStmt::generateStructure(new_ctx, IRNodeKind::IF_ELSE);
...
void IfElseStmt::populate(std::shared_ptr<PopulateCtx> ctx) {
    cond = ArithmeticExpr::create(ctx, IntTypeID::UCHAR, true);

    if (!cond->getValue()->isScalarVar()) {
        ERROR("Can perform conversion to bool only on scalar variables");
    }

    std::shared_ptr<IntegralType> int_type = std::static_pointer_cast<IntegralType>(cond->getValue()->getType());
    ...
    IRValue cond_val = std::static_pointer_cast<ScalarVar>(cond_eval_res)->getCurrentValue();
...
void StubStmt::emit(std::shared_ptr<EmitCtx> ctx, std::ostream &stream,
                    std::string offset) {
    stream << offset << text;
    std::cout << "Stub stmt #" + text << std::endl;
}
