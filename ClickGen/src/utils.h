...
class RandValGen {
  public:
    ...
    IntTypeID getHeaderFieldTypeID(std::string new_name);
    std::string getHeaderypeBelowID(IntTypeID type);
    ...
    std::string getNNName(std::string new_name);

    std::vector<std::pair<std::string, IntTypeID>> allFields;
    std::vector<std::pair<std::string, std::string>> click2nn;
    std::stack<std::set<std::string>> hfCtxStack;
    std::stack<uint32_t> stmtNumStack;
    uint32_t totalAvailHF = 0;
    bool varAllUsed = false;
    bool hfAllUsed = false;
