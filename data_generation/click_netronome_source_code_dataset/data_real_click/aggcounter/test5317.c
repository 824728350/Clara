...
// global variables
__volatile __shared __lmem uint32_t AGGREGATE_ANNO, EXTRA_PACKETS_ANNO, EXTRA_LENGTH_ANNO;
__volatile __shared __lmem uint32_t _bytes = 0, _use_packet_count = 1, _use_extra_length = 1, _ip_bytes;
__volatile __shared __lmem uint32_t _num_nonzero, _call_nnz, _call_nnz_h, _frozen, _count = 0, _call_count = 0, _call_count_h;
...
void pkt_count_rx(__mem40 char **buf_addr, uint32_t *buf_off,
  ...
  __lmem int i = 0, j = 0, k = 0;

  // local variables
  __volatile __lmem uint32_t n = 0, agg, amount, frozen = 0;
  k = 0;
  for (; k < BATCH_SIZE; k++) {
    mem_incr64(&cntrs->rx);
    src_off = buf_off[k];
