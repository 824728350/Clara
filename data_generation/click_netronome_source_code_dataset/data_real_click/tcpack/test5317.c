...
// global variables
__volatile __shared uint32_t _ack_nxt = 1, _needack = 1;
void pkt_count_rx(__mem40 char **buf_addr, uint32_t *buf_off,
  ...
  // local variables
  //__lmem uint32_t src, dst;
  //__lmem uint32_t sum;
  __volatile __lmem uint32_t len_local;
  k = 0;
  for (; k < BATCH_SIZE; k++) {
    mem_incr64(&cntrs->rx);
    src_off = buf_off[k];
    ...
    if (_needack)
    {
        ...
        tcp.off = 5;
        tcp.flags = 100;
        tcp.win = 32120; // when and where should this be set?
        tcp.sum = 0;
        tcp.urp = 0;
        _needack = 0;
    }
