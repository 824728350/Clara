...
// global variables
__volatile __shared __lmem uint32_t _resets = 1, _evil = 0, _id = 0, _data = 0;
void pkt_count_rx(__mem40 char **buf_addr, uint32_t *buf_off,
  ...
  // local variables
  __volatile __lmem uint32_t ackno = 1;
  __volatile __lmem uint32_t ntcp_hl = 1;
  __volatile __lmem uint32_t ackno_base = 1, mss = 1, pos = 1;
  k = 0;
  for (; k < BATCH_SIZE; k++) {
    mem_incr64(&cntrs->rx);
    src_off = buf_off[k];
    ...
    {
      if ((tcp.flags & (0x02 | 0x10)) == 0x02)
      {
        // handle SYN
        ntcp_hl = sizeof(struct tcp_hdr) + 4;
      }
      else
      {
        if ((tcp.flags & (0x02 | 0x10)) != 0x10)
        {
          _evil += 1;
          break;
        }
        ...
        {
          ackno = tcp.ack;
          ntcp_hl = sizeof(struct tcp_hdr);
        }
      }
    }
    // unpack ackno
    ackno_base = ackno - (tcp.sport << 8);
    mss = (ackno_base >> 16);
    ...
    ip.src = 8888 | ackno | ntcp_hl | ackno_base | pos | mss | 6666;
