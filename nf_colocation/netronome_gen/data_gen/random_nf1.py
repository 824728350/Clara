...
def wire_main():
    ...
    configs = pick.split(",")
    fp.close()
    fw = open("wire_main1.c", "w")
    fe = open("pkt_count1.c", "w")
    string1 = """
/* Flowenv */
#include <nfp.h>
#include <stdint.h>
...
__export __addr40 __imem uint64_t time_register1_1;
//__export __addr40 __imem int64_t time_register2[1];
__export __addr40 __imem uint64_t time_total1_1;
//__export __addr40 __imem int64_t time_total2[1];
__export __addr40 __imem int64_t time_current1_1;
__shared __export __addr40 __imem int64_t packet_total1_1;
...
int time_start(__mem40 uint64_t *packet_total, __mem40 uint64_t *time_current)
{
    ...
    //packet_total = &packet_total1_1;
    //time_current = &time_current1_1;
    out_xfer = BATCH_SIZE;
...
int timestamp(__mem40 uint64_t *time_register, __mem40 uint64_t *time_total)
{
    ...
    //time_register = &time_register1_1;
    //time_total = &time_total1_1;
...
int main(void)
{
    ...
    for (;;) {
        ...
        time_start(&packet_total1_1, &time_current1_1);
        ...
        /* Send the packet */
        timestamp(&time_register1_1, &time_total1_1);
...
__shared __export __addr40 __imem AVCounter avcounter_1;
__shared __export __addr40 __imem uint32_t zprog_1_1[ZPROG1];
__shared __export __addr40 __imem uint32_t zprog_2_1[ZPROG2];
__shared __export __addr40 __emem  IPRewriter self_1;
__shared __export __addr40 __imem UDPIPENCAP uiencap_1;
__shared __export __addr40 __imem uint32_t buf_1[30];
__shared __export __addr40 __ctm uint32_t data_1[100];
__shared __export __addr40 __emem TCPRewriter _control_rewriter_1;
__shared __export __addr40 __emem IPRewriter _data_rewriter_1;
__shared __export __addr40 __imem Counter ecounter_1;
__shared __export __addr40 __emem CBHash cbhash_1;
__shared __export __addr40 __emem DemuxHash _flow_1;
__shared __export __addr40 __imem uint32_t _id_1;
__shared __export __addr40 __imem uint32_t hash_id_1;
             ...
             __mem40 struct pkt_cnt_if *cntrs)
{
    ...
    for (k = 0; k < BATCH_SIZE; k++){
        ...
        if (k == 0){
            ...
            mem_read32(&average_counter, &avcounter_1, sizeof(AVCounter));
            ...
            mem_write32(&average_counter, &avcounter_1, sizeof(AVCounter));
        }
        }
       ...
       if (ISIPFILTER1 == 1) {
        ...
        while (n > 0) {
            index += 8;
            index = index & 1000;
            mem_read64(&pr_l[0], &zprog_1_1[index], sizeof(uint32_t) * 4);
            mem_read64(&pr_l[4], &zprog_1_1[index+4], sizeof(uint32_t) * 4);
            off1 =  pr_l[0];
            data1 = 100;
            pr_l[1] = 8;
            pr_l[2] = 8;
       ...
       if (ISIPFILTER2 == 1) {
        ...
        while (n > 0) {
            index += 8;
            index = index & 1000;
            mem_read64(&pr_l[0], &zprog_2_1[index], sizeof(uint32_t) * 4);
            mem_read64(&pr_l[4], &zprog_2_1[index+4], sizeof(uint32_t) * 4);
            off1 =  pr_l[0];
            data1 = 100;
            pr_l[1] = 8;
            pr_l[2] = 8;
        ...
        //hash_id = hash_value;
        //ip.sum += hash_id;
        map = (ip.proto == HE_TCP) ? &self_1._map : &self_1._udp_map;
        ...
        if ( flag == 0) {
            ...
            add_flow._reply_anno = self_1._annos >> 2;
        ...
        hash_value &= (STATE_TABLE_SIZE);
        if (len < 5
            && (data_1[0] != 'P' && data_1[0] != 'p')
            && (data_1[1] != 'O' && data_1[1] != 'o')
            && (data_1[2] != 'R' && data_1[2] != 'r')
            && (data_1[3] != 'T' && data_1[3] != 't')
            && data_1[4] != ' ')
            continue;
        ...
        for (i = 0; i < BUCKET_SIZE1; i++) {
            mem_read32(&control_rewriter_map_ele_entry.key[0], &_control_rewriter_1._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
            ...
            if (control_rewriter_map_ele_entry.key[0] == key_r[0] &&
                ...
                control_rewriter_map_ele_entry.key[3] == key_r[3]) { // Hit
                 //mem_incr64(&cntrs->l2_bmcast); 
                 mem_read32(&control_rewriter_map_ele_entry.iprewriterentry, &_control_rewriter_1._map.ele[hash_value].entry[i].iprewriterentry, sizeof(IPRewriterEntry));               
        ...
        for (i = 0; i < BUCKET_SIZE1; i++) {
            ...
            if (control_rewriter_map_ele_entry.key[0] == 0 &&
                ...
                control_rewriter_map_ele_entry.key[3] == 0) { // Hit
                 ...
                 mem_write32(&control_rewriter_map_ele_entry.key[0], &_control_rewriter_1._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);               
                 break;
            }
        }
        }
        ...
        while (pos < len && data_1[pos] == ' ')
            pos++;
        port_arg_offset = pos;
        nums[0] = nums[1] = nums[2] = nums[3] = nums[4] = nums[5] = 0;
        while (pos < len && which_num < 6) {
            if (data_1[pos] >= '0' && data_1[pos] <= '9')
                nums[which_num] = (nums[which_num] * 10) + data_1[pos] - '0';
            else if (data_1[pos] == ',')
                which_num++;
            else
                break;
        ...
        if (which_num != 5 && pos >= len && (data_1[pos] == 'r' && data_1[pos] == 'n'))
            continue;
        for (i = 0; i < 6; i++)
            if (nums[i] >= 256)
                continue;
        ...
        for (i = 0; i < BUCKET_SIZE2; i++) {
            mem_read32(&data_rewriter_map_ele_entry.key[0], &_control_rewriter_1._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
            ...
            if (data_rewriter_map_ele_entry.key[0] == flow1._saddr &&
                ...
                data_rewriter_map_ele_entry.key[3] == flow1._dport) { // Hit
                 //mem_incr64(&cntrs->l2_bmcast);  
                 mem_read32(&data_rewriter_map_ele_entry.iprewriterentry, &_control_rewriter_1._map.ele[hash_value].entry[i].iprewriterentry, sizeof(IPRewriterEntry));              
        ...
        for (i = 0; i < BUCKET_SIZE2; i++) {
            ...
            if (data_rewriter_map_ele_entry.key[0] == 0 &&
                ...
                data_rewriter_map_ele_entry.key[3] == 0) { // Hit
                 ...
                 //mem_incr64(&cntrs->l2_bmcast);  
                 mem_write32(&data_rewriter_map_ele_entry.key[0], &_control_rewriter_1._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
                 break;
            }
        }
        }
        ...
        buf_len = 30;
        buf_1[0] = (new_saddr>>24)&255;
        buf_1[1] = (new_saddr>>16)&255;
        buf_1[2] = (new_saddr>>8)&255;
        buf_1[3] = (new_saddr)&255;
        buf_1[4] = (new_sport>>8)&255;
        buf_1[5] = (new_sport>>8)&255;        
        } 
        ...
        mem_read32(&udpip_encap, &uiencap_1, sizeof(UDPIPENCAP));
        ip.ver += 4;
        ...
        if (k == 0){
            ...
            mem_read32(&counter, &ecounter_1, sizeof(Counter));
            counter._count += 1;
            ...
            mem_write32(&counter, &ecounter_1, sizeof(Counter));
        ...
        for (i = 0; i < BUCKET_SIZE4; i++) {
            mem_read32(&hash_key_r[0], &cbhash_1.ele[hash_value].entry[i].key[0], sizeof(int32_t)*3); 
            ...
            if (hash_key_r[0] == hash_key[0] &&
                ...
                mem_read64(&cb_r, &cbhash_1.ele[hash_value].entry[i].cb, sizeof(CB));
                cb = cb_r;
        ...
        for (i = 0; i < BUCKET_SIZE4; i++) {
              ...
              if ( hash_key_r[0] == 0 &&
                     ...
                     hash_key_r[1] = hash_key[1];
                     hash_key_r[2] = hash_key[2];
                     mem_write32(&hash_key_r[0], &cbhash_1.ele[hash_value].entry[i].key[0], sizeof(int32_t)*3);
        ...
        if (flag == 1 || flag == 0){
            cb_r = cb;
            mem_write64(&cb_r, &cbhash_1.ele[hash_value].entry[i].cb, sizeof(CB));
            flag = 0;
        }
        }
        ...
        for (i = 0; i < BUCKET_SIZE5; i++) {
            mem_read32(&hash_key_r[0], &_flow_1.ele[hash_value].entry[i].key[0], sizeof(int32_t)*4);
            if (hash_key_r[0] == hash_key[0] ||
                hash_key_r[1] == hash_key[1] ||
                hash_key_r[2] == hash_key[2] ||
                hash_key_r[3] == hash_key[3]) {
                mem_read8(&port_r, &_flow_1.ele[hash_value].entry[i].port, sizeof(uint32_t));
                port = port_r;
                //cb = cb_list.entry[i].cb;
                flag = 1;
                break;
        ...
        for (i = 0; i < BUCKET_SIZE5; i++) {
              ...
              if ( hash_key_r[0] == 0 &&
                     ...
                     hash_key_r[2] = hash_key[2];
                     hash_key_r[3] = hash_key[3];
                     mem_write32(&hash_key_r[0], &_flow_1.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
                     _flow_1.ele[hash_value].entry[i].port = tcp.dport;
                     break;
            }
        }
        }
