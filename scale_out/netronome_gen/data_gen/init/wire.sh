...
case "$1" in
    start)
        ...
        #nfp -m mac -e set port rx 0 0 enable || exit 1
        ...
        nfp-nsp -C +rx eth3
        nfp -m mac -e set port rx 0 0 enable || exit 1
        nfp -m mac -e set port rx 0 1 enable || exit 1
        nfp -m mac -e set port rx 0 2 enable || exit 1
        nfp -m mac -e set port rx 0 3 enable || exit 1
        echo "done"

        echo -n " - Set EgressPrependEnable..."
        nfp-reg xpb:Nbi0IsldXpbMap.NbiTopXpbMap.MacGlbAdrMap.MacCsr.EgCmdPrependEn0Lo.EgCmdPrependEn=0xf000f || exit 1
        echo "done"
    ...
    stop)
        ...
        echo "Stopping FW:"
        (rmmod nfp || true) 2> /dev/null || exit 1
        #sleep 1
