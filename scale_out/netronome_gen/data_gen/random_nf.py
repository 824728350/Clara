...
__shared __export __addr40 __imem AVCounter avcounter;
__shared __export __addr40 __imem uint32_t zprog_1[ZPROG1];
__shared __export __addr40 __imem uint32_t zprog_2[ZPROG2];
__shared __export __addr40 __emem  IPRewriter self;
__shared __export __addr40 __imem UDPIPENCAP uiencap;
__shared __export __addr40 __imem uint32_t buf[30];
__shared __export __addr40 __ctm uint32_t data[100];
...
__shared __export __addr40 __imem Counter ecounter;
__shared __export __addr40 __emem CBHash cbhash;
__shared __export __addr40 __emem DemuxHash _flow;
__shared __export __addr40 __imem uint32_t _id;
__shared __export __addr40 __imem uint32_t hash_id;
__shared __export __addr40 __imem uint32_t imem_var0;
__shared __export __addr40 __imem uint32_t imem_var1;
__shared __export __addr40 __imem uint32_t imem_var2;
__shared __export __addr40 __imem uint32_t imem_var3;
__shared __export __addr40 __imem uint32_t imem_var4;
/*
             ...
             __mem40 struct pkt_cnt_if *cntrs)
{
    ...
    for (k = 0; k < BATCH_SIZE; k++){
        ...
        if (k == 0){
            ...
            mem_read32(&average_counter, &avcounter, sizeof(AVCounter));
            ...
            average_counter.rate = average_counter._byte_count * (average_counter._last-avcounter._first);
            ip.src += average_counter.rate;
            average_counter._first = 0;
            mem_write32(&average_counter, &avcounter, sizeof(AVCounter));
        }
        }
       ...
       if (ISCOMPUTE2 == 1){
        ...
        if (ISIMEM2 == 1){
            int o;
            for (o = 0; o < TUNEIMEM2; o++){
                imem_var2 += ip.ttl;
            }
        }
       //filter 1;
       if (ISIPFILTER1 == 1) {
        __mem40 uint32_t *pr = zprog_1;
        __mem40 uint32_t *pp;
        __mem40 uint32_t *px;
        __mem40 uint32_t *pm;
        uint32_t off1;
        ...
        while (n > 0) {
            index += 8;
            index = index & 1000;
            mem_read64(&pr_l[0], &zprog_1[index], sizeof(uint32_t) * 4);
            mem_read64(&pr_l[4], &zprog_1[index+4], sizeof(uint32_t) * 4);
            //mem_read8(&pr_l[0], &pr[0], sizeof(uint32_t) * 4);
            //mem_read8(&pr_l[4], &pr[4], sizeof(uint32_t) * 4);
            //mem_read64(&pr_l[8], &pr[8], sizeof(uint32_t) * 8);
            off1 =  pr_l[0];
            data1 = 100;
            pr_l[1] = 8;
            pr_l[2] = 8;
       ...
       if (ISIPFILTER2 == 1) {
        ...
        while (n > 0) {
            index += 8;
            index = index & 1000;
            mem_read64(&pr_l[0], &zprog_2[index], sizeof(uint32_t) * 4);
            mem_read64(&pr_l[4], &zprog_2[index+4], sizeof(uint32_t) * 4);
            off1 =  pr_l[0];
            data1 = 100;
            pr_l[1] = 8;
            pr_l[2] = 8;
        ...
        //hash_id = hash_value;
        //ip.sum += hash_id;
        map = (ip.proto == HE_TCP) ? &self._map : &self._udp_map;
        ...
        for (i = 0; i < BUCKET_SIZE1; i++) {
            mem_read32(&control_rewriter_map_ele_entry.key[0], &_control_rewriter._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
            ...
            if (control_rewriter_map_ele_entry.key[0] == key_r[0] &&
                ...
                control_rewriter_map_ele_entry.key[3] == key_r[3]) { // Hit
                 //mem_incr64(&cntrs->l2_bmcast); 
                 mem_read32(&control_rewriter_map_ele_entry.iprewriterentry, &_control_rewriter._map.ele[hash_value].entry[i].iprewriterentry, sizeof(IPRewriterEntry));               
                 flag = 1;
        ...
        for (i = 0; i < BUCKET_SIZE1; i++) {
            ...
            if (control_rewriter_map_ele_entry.key[0] == 0 &&
                ...
                control_rewriter_map_ele_entry.key[3] == 0) { // Hit
                 ...
                 mem_write32(&control_rewriter_map_ele_entry.key[0], &_control_rewriter._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);               
                 break;
            }
        }
        }
        ...
        for (i = 0; i < BUCKET_SIZE2; i++) {
            mem_read32(&data_rewriter_map_ele_entry.key[0], &_control_rewriter._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
            ...
            if (data_rewriter_map_ele_entry.key[0] == flow1._saddr &&
                ...
                data_rewriter_map_ele_entry.key[3] == flow1._dport) { // Hit
                 //mem_incr64(&cntrs->l2_bmcast);  
                 mem_read32(&data_rewriter_map_ele_entry.iprewriterentry, &_control_rewriter._map.ele[hash_value].entry[i].iprewriterentry, sizeof(IPRewriterEntry));              
        ...
        for (i = 0; i < BUCKET_SIZE2; i++) {
            ...
            if (data_rewriter_map_ele_entry.key[0] == 0 &&
                ...
                data_rewriter_map_ele_entry.key[3] == 0) { // Hit
                 ...
                 //mem_incr64(&cntrs->l2_bmcast);  
                 mem_write32(&data_rewriter_map_ele_entry.key[0], &_control_rewriter._map.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
                 break;
            }
        }
        }
        ...
        mem_read32(&udpip_encap, &uiencap, sizeof(UDPIPENCAP));
        ip.ver += 4;
        ...
        if (k == 0){
            ...
            mem_read32(&counter, &ecounter, sizeof(Counter));
            counter._count += 1;
            ...
            mem_write32(&counter, &ecounter, sizeof(Counter));
        ...
        for (i = 0; i < BUCKET_SIZE4; i++) {
            mem_read32(&hash_key_r[0], &cbhash.ele[hash_value].entry[i].key[0], sizeof(int32_t)*3); 
        ...
        for (i = 0; i < BUCKET_SIZE4; i++) {
              ...
              if ( hash_key_r[0] == 0 &&
                     ...
                     hash_key_r[1] = hash_key[1];
                     hash_key_r[2] = hash_key[2];
                     mem_write32(&hash_key_r[0], &cbhash.ele[hash_value].entry[i].key[0], sizeof(int32_t)*3);
        ...
        for (i = 0; i < BUCKET_SIZE5; i++) {
            mem_read32(&hash_key_r[0], &_flow.ele[hash_value].entry[i].key[0], sizeof(int32_t)*4);
            if (hash_key_r[0] == hash_key[0] ||
                hash_key_r[1] == hash_key[1] ||
                hash_key_r[2] == hash_key[2] ||
                hash_key_r[3] == hash_key[3]) {
                mem_read8(&port_r, &_flow.ele[hash_value].entry[i].port, sizeof(uint32_t));
                port = port_r;
                //cb = cb_list.entry[i].cb;
                flag = 1;
                break;
        ...
        for (i = 0; i < BUCKET_SIZE5; i++) {
              ...
              if ( hash_key_r[0] == 0 &&
                     ...
                     hash_key_r[2] = hash_key[2];
                     hash_key_r[3] = hash_key[3];
                     mem_write32(&hash_key_r[0], &_flow.ele[hash_value].entry[i].key[0], sizeof(uint32_t) * 4);
                     _flow.ele[hash_value].entry[i].port = tcp.dport;
                     break;
            }
        }
        }
