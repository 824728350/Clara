...
__intrinsic void
pkt_nbi_recv_with_hdrs_sig_done(__xread void *meta, size_t msize, uint32_t off)
{
    SIGNAL add_thread_sig;
    __pkt_nbi_recv_with_hdrs(meta, msize, off, sig_done, &add_thread_sig);
}

__intrinsic void
pkt_nbi_recv_sig_done(__xread void *meta, size_t msize)
{
    pkt_nbi_recv_with_hdrs_sig_done(meta, msize, 0);
}
