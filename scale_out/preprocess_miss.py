...
def feature_extraction(file_name, length): 
    ...
    for index in range(len(features)):
        ...
        emem_bool = [2.0*iprewriter, 2.0*ftpportmapper1, 2.0*ftpportmapper2, 1.0*webgen, 2.0*demux, 2.0*conn]
        ...
        compute_bool = [25*sizeaccelerate, 10*sizecompute, 60*ipfilter1, 60*ipfilter2, 35*iprewriter+20, 35*ftpportmapper1+20, 35*ftpportmapper2+20, 12*isconn, 18*demux, 40*avcounter, 20*counter, 60*udpipencap, 16*webgen+50]
        #compute_bool = [25*sizeaccelerate, 10*sizecompute, 100*ipfilter1, 100*ipfilter2, 20*isiprewriter+20, 20*isftpportmapper1+20, 20*isftpportmapper2+20, 12*isconn, 18*isdemux, 40*avcounter, 20*counter, 60*udpipencap, 10*iswebgen+50]
        ...
        #features[index] += [sum(compute_logic)/(sum(compute_feature) - sum(compute_logic))] # Compute intensity of non-stareftul data structure elements/stateful data structure elements.
        ...
        features[index] += [(sum(emem_bool) + sum(imem_feature))/sum(compute_bool)]
        features[index] += [(sum(imem_feature))/ (330*sum(emem_bool)+sum(compute_bool)+55*sum(imem_feature))]
        features[index] += [(sum(emem_bool))/ (330*sum(emem_bool)+sum(compute_bool)+55*sum(imem_feature))]    
        features[index] += [(1.0*sum(imem_feature)+6.0*sum(emem_bool))/sum(compute_bool)]
        features[index] += [(1.0*sum(imem_feature)-6.0*sum(emem_bool))/sum(compute_bool)]
        #features[index] += [(1.0*sum(imem_feature)+1.6*sum(emem_bool))/sum(compute_bool)]
        #features[index] += [(1.0*sum(imem_feature)-1.6*sum(emem_bool))/sum(compute_bool)]
...
if __name__=='__main__':
    ...
    features, tags = feature_extraction(file_name, 445)
    features_train = np.array(features[:320])
    tags_train = np.array(tags[:320])
    features_test = np.array(features[320:])
    tags_test = np.array(tags[320:])
