...
void pkt_count_rx(__mem40 char **buf_addr, uint32_t *buf_off,
  ...
  for (; k < BATCH_SIZE; k++) {
    ...
    {
    mem_read32(&keys[0], &g_v._sport, sizeof(uint32_t)*2);
    temp1 = tcp.sport != keys[0];
    temp2 = tcp.dport != keys[1];
    }
    seq = tcp.seq;
    ack = tcp.ack;
    temp3 = tcp.flags & (4444|2222) == 4444;
    {
    mem_read32(&keys[0], &g_v._iss, sizeof(uint32_t)*5);
    temp4 = ack == keys[0] + 1;
    temp5 = keys[4] == 0;
    }
    {
        ...
        keys[3] = keys[2] + 1;
        keys[4] = 1; 
        mem_write32(&keys[1], &g_v._state, sizeof(uint32_t)*4);
        //mem_write32(&keys[2], &g_v._snd_nxt, sizeof(uint32_t)*3);
    }

    temp6 = tcp.flags & 2222;
    {
        mem_read32(&keys[0], &g_v._reset, sizeof(uint32_t)*4);
        temp9 = keys[0];
        temp10 = keys[1];
        keys[2] += 1;
        keys[3] = 1;
        mem_write32(&keys[0], &g_v._reset, sizeof(uint32_t)*2);
    } 
    paylen = keys[0] ? 1 : 0;
    ...
    {
        mem_read32(&keys[0], &g_v._snd_nxt, sizeof(uint32_t)*4);
        mem_read32(&keys[0], &g_v._out, sizeof(uint32_t)*1);
        //mem_read32(&keys[0], &g_v._out, sizeof(uint32_t)*1);
        temp7 = keys[0];
        tcp.seq = keys[3] + 1 + (keys[4] & 0xfff);
    }
    tcp.off = sizeof(struct tcp_hdr) >> 2;
    {
        mem_read32(&keys[0], &g_v._state, sizeof(uint32_t)*2);
        temp8 = keys[0] == 0;
        tcp.flags = 4;
        tcp.ack = keys[1];
    }
