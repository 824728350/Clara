...
// global variables
//__shared __export __addr40 __imem CBHash cbhash;
...
__volatile __shared __export __addr40 __imem GV cb;
...
void pkt_count_rx(__mem40 char **buf_addr, uint32_t *buf_off,
  ...
  for (; k < BATCH_SIZE; k++) {
    ...
    //packet header manipulations 
    {
        ...
        if (cb._connected && cb._snd_una - cb._iss - 1 < cb._sndlen) {
            paylen = cb._sndlen;
            seq = cb._iss + 1;
        ...
        {
          if (seq + dlen == cb._rcv_nxt && cb._got_fin == 0) {
            cb._got_fin = 1;
            cb._rcv_nxt += 1;
            cb._do_send = 1;
          }
