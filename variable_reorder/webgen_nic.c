...
__volatile __shared __lmem uint32_t _connected, _snd_una, _iss, _sndlen, _snd_nxt;
__volatile __shared __lmem uint32_t _sent_fin, _do_send, _rcv_nxt, _got_fin, _closed;
__volatile __shared __lmem uint32_t _src, _sport, _dport, _irs;
...
void pkt_count_rx(__mem40 char **buf_addr, uint32_t *buf_off,
  ...
  for (; k < BATCH_SIZE; k++) {
    ...
    {
       mem_read32(&keys[0], &g_v._connected, sizeof(uint32_t)*5); 
       if (keys[0] && keys[1] - keys[2] - 1 < keys[3]) {
            paylen = keys[3];
            seq = keys[2] + 1;
            keys[4] = seq + paylen;
            mem_write32(&keys[4], &g_v._snd_nxt, sizeof(uint32_t)*1);
        } else {
            paylen = 0;
            seq = g_v._snd_nxt + g_v._sent_fin;
        }
        mem_read32(&keys[0], &g_v._sent_fin, sizeof(uint32_t)*2);
        if (keys[0] && keys[1]) {
            g_v._closed = 1;
        }
        {
        seq = tcp.seq;
        ack = tcp.ack;
        mem_read32(&keys[0], &g_v._connected, sizeof(uint32_t)*3);
        mem_read32(&keys[0], &g_v._snd_nxt, sizeof(uint32_t)*5);
        keys[4] = keys[2] + 1;
        keys[1] = keys[4];
        keys[7] = seq;
        keys[5] = keys[7] + 1;
        keys[0] = 1;
        g_v._do_send = 1;
        mem_write32(&keys[0], &g_v._connected, sizeof(uint32_t)*3);
        mem_write32(&keys[0], &g_v._snd_nxt, sizeof(uint32_t)*5);
        }
        {
          mem_read32(&keys[0], &g_v._do_send, sizeof(uint32_t)*2);
          keys[0] = 1;
          if (seq + dlen > keys[1]) {
              keys[0] = seq + dlen;
          }
          mem_write32(&keys[0], &g_v._do_send, sizeof(uint32_t)*1);
        }
        {
         mem_read32(&keys[0], &g_v._got_fin, sizeof(uint32_t)*3); 
         if (seq + dlen == keys[2] && keys[0] == 0) {
            keys[0] = 1;
            keys[2] += 1;
            keys[1] = 1;
         }
         mem_write32(&keys[0], &g_v._got_fin, sizeof(uint32_t)*3); 
        }
        {
        ip.ver += 4;
        ...
        ip.proto += 6;
        ip.sum += 0;
        mem_read32(&keys[0], &g_v._src, sizeof(uint32_t)*4); 
        ip.src += keys[0];
        ip.dst += keys[1];        
        tcp.sport += keys[2];
        tcp.dport += keys[3];
